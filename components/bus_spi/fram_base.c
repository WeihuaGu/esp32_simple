...
esp_err_t send_WRDI(){
    ...
    ret = send_command_array(&wrdi_cmd, 1);
    return ret;
...
void fram_test(){
    ...
    // 写入数据到 FRAM
    uint8_t write_data = 0x41; // 要写入的数据
    if (fram_write(addr, &write_data, 1) == ESP_OK) {
        ESP_LOGI("SPI", "Data written to FRAM at address 0x%hu: %x", addr, write_data);
    } else {
    ...
    if (fram_read(addr, &read_data, 1) == ESP_OK) {
        ESP_LOGI("SPI", "Data read from FRAM at address 0x%hu: %x", addr, read_data);
    } else {
